...
function evoproject_execute($argv) {
    $options = [];
    $key = array_search('-f', $argv);
    if ($key !== false) {
        $items = array_splice($argv, $key, 2);
        $options['f'] = $items[1];
        print_r($items);
    }
    ...
    if (!file_exists($evoProjectJsonPath)) {
        echo("Can't find '{$evoProjectJsonPath}'\n");
        return -1;
    }
    ...
    if (count($argv) < 0 || !method_exists($evoProject, $target)) {
